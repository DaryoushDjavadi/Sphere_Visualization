...
async function main() {
  ...
  await page.locator('#bring-list').fill('WG Einkauf')
  await page.getByRole('button', { name: /Bring verknüpfen/i }).click()
  await page.getByText('Verknüpft').waitFor()
  await page.getByRole('button', { name: 'Cookidoo umschalten' }).click()
  await page.getByRole('button', { name: /Cookidoo freischalten|Cookidoo bereit/i }).click()
  ...
  await page.locator('#c-url').fill('https://cookidoo.de/recipes/recipe/de-DE/demo123')
  ...
  await page.locator('.flash').filter({ hasText: 'Demo-Push' }).waitFor()
  await shot(page, '06-wendy-bring-push')
  log.push('Scenario 2 PASS: Wendy settings + Cookidoo import + Bring push')
